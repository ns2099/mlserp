...
export default function MakinaDuzenlePage() {
  ...
  useEffect(() => {
    ...
    // Makina bilgilerini yükle
    fetch(`/api/makina/${id}`)
      .then((res) => res.json())
      .then((data: MakinaData) => {
        setFormData({
          ad: data.ad || '',
          model: data.model || '',
        ...
        // Bileşenleri yükle
        if (data.makinaBilesenleri && data.makinaBilesenleri.length > 0) {
          setBilesenler(
            data.makinaBilesenleri.map((b) => ({
              id: b.id,
              ad: b.ad,
              aciklama: b.aciklama || '',
              miktar: b.miktar,
              ...
              toplamMaliyet: b.miktar * b.birimMaliyet,
              toplamMaliyetEUR: b.toplamMaliyet,
            }))
          )
        }
