...
interface TeklifMeta {
  ...
  paraBirimi?: string
  toplamMaliyet?: number
  toplamFiyatBrut?: number
  genelIskonto?: number
  genelIskontoTuru?: IskontoTuru
  toplamFiyatNet?: number
...
export default function TeklifDuzenlePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/teklif/${id}`)
      .then((res) => res.json())
      .then((data) => {
        // Meta bilgilerini parse et
        let meta: TeklifMeta = {}
        try {
          if (data.aciklama) {
            meta = JSON.parse(data.aciklama)
          }
        } catch {
        ...
        setFormData({
          ...
          konu: meta.konu || '',
          not: meta.not || '',
          odemeSekli: meta.odemeSekli || '',
          paraBirimi: meta.paraBirimi || 'TRY',
          aciklama: typeof data.aciklama === 'string' && !meta.konu ? data.aciklama : '',
          teklifTarihi: data.teklifTarihi ? data.teklifTarihi.split('T')[0] : '',
          durum: data.durum || 1,
        })
        ...
        if (meta.iletisim) {
          setIletisim({
            ticariUnvan: meta.iletisim.ticariUnvan || '',
            yetkiliAdSoyad: meta.iletisim.yetkiliAdSoyad || '',
            telefon: meta.iletisim.telefon || '',
            email: meta.iletisim.email || '',
            vergiKimlikNo: meta.iletisim.vergiKimlikNo || '',
            vergiDairesi: meta.iletisim.vergiDairesi || '',
            adres: meta.iletisim.adres || '',
          })
        }

        // Genel iskonto
        if (meta.genelIskonto !== undefined) {
          setGenelIskonto(meta.genelIskonto)
        }
        if (meta.genelIskontoTuru) {
          setGenelIskontoTuru(meta.genelIskontoTuru)
        }
        ...
        // Ürünleri doldur
        if (data.teklifUrunler && data.teklifUrunler.length > 0) {
          const urunDetay = meta.urunDetay || []
          setUrunler(
            data.teklifUrunler.map((u: any, index: number) => {
              const detay = urunDetay[index] || {}
              const urun: TeklifUrun = {
                id: u.id,
                urunAdi: u.urunAdi || '',
                birim: detay.birim || 'Adet',
                miktar: u.miktar || 1,
                birimMaliyet: detay.birimMaliyet || 0,
                birimFiyat: u.birimFiyat || 0,
                kdv: detay.kdv || 0,
                iskonto: detay.iskonto || 0,
                iskontoTuru: detay.iskontoTuru || 'yuzde',
                toplamMaliyet: detay.toplamMaliyet || 0,
                toplamFiyat: u.toplamFiyat || u.miktar * u.birimFiyat,
                aciklama: u.aciklama || '',
              }
              return urun
            })
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-sm text-gray-500">Düzenleme</p>
